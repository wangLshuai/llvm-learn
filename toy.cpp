...
Value *BinaryExprAST::codegen() {
  ...
  if (Op == '=') {
    ...
    if (!LHSE)
      return LogErrorV("destination of '=' must be a variabler");
...
Value *ForExprAST::codegen() {
  ...
  Builder->CreateStore(NextVar, Alloca, false);
...
Function *FunctionAST::codegen() {
  ...
  // Create a new subprogram DIE for this function.
  ...
  for (auto &Arg : TheFunction->args()) {
    ...
    // Creae a debug descriptor for the variable.
    DILocalVariable *D = DBuilder->createParameterVariable(
        SP, Arg.getName(), Idx, Unit, LineNo, KSDbgInfo.getDoubleTy(), true);
...
Value *VarExprAST::codegen() {
  ...
  for (unsigned i = 0, e = VarNames.size(); i != e; ++i)
    NamedValues[VarNames[i].first] =
        OldBindings[i]; // ？？ null is supposed to erase?
...
// Var identify (= expr)? (, identify (= expr)?)? 'in' expression;
static std::unique_ptr<ExprAST> ParseVarExpr() {
  ...
  if (CurTok != tok_identifier)
    return LogError("expected identifer after var");
  ...
  // At this spoint, we have to hane 'in'
  if (CurTok != tok_in)
    return LogError("expected identifier list after var");
  getNextToken(); // eat in
...
static std::unique_ptr<PrototypeAST> ParsePrototype() {
  ...
  switch (CurTok) {
  default:
    return LogErrorP("Expected function name in progotype");
  case tok_identifier:
  ...
  case tok_binary:
    getNextToken();
    if (!isascii(CurTok)) // operator LETTER
  ...
  return std::make_unique<PrototypeAST>(CurLoc, FnName, std::move(ArgNames),
...
static std::unique_ptr<FunctionAST> ParseTopLevelExpr() {
  if (auto E = ParseExpression()) {
    ...
    auto Proto = std::make_unique<PrototypeAST>(CurLoc, "main",
